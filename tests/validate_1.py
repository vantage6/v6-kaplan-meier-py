...
# compute the kaplan meier curve for the combined dataset
from lifelines import KaplanMeierFitter

kmf = KaplanMeierFitter(label="dummy data")
kmf.fit(df["TIME_AT_RISK"], df["MORTALITY_FLAG"])
...
# Plot the Kaplan-Meier curve for clean data
ax2.plot(
    df_events_clean["TIME_AT_RISK"], df_events_clean["survival_cdf"], label="Federated"
)
ax2.plot(
    kmf.survival_function_.index,
    kmf.survival_function_["dummy data"],
    label="Centralized",
    linestyle="--",
)
